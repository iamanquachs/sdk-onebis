    ...
    return function (atomic) {
        return function (attribute) {
            ...
            var defaultTags = atomic.tags
                .filter(function (row) {
                return row.some(function (tag) { return tag.kind === "default" || tag.schema !== undefined; });
            })
                .map(function (row) {
                return row.filter(function (tag) { return tag.kind === "default" || tag.schema !== undefined; });
            })
                .flat();
            if (defaultTags.length === 0)
                return [base];
            return defaultTags.map(function (tag) { return (__assign(__assign(__assign(__assign({}, base), (tag.kind === "default"
                ? {
                    default: tag.value,
                }
                : {})), (options.surplus
